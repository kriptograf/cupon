...
class AdsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('index','view','print','complain','category','search'),
				'users'=>array('*'),
			),
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','upload','preview'),
				'users'=>array('@'),
			),
    ...
    public function actionSearch()
    {
        ...
        if($_POST)
        {
            Yii::app()->session['ads_search'] = $_POST;
        }
        ...
        if($_POST['date_range'])
        {
            $range = CHtml::encode($_POST['date_range']);
            if($range === 'week')
            {
                $cond = time()-604800;
                $criteria->compare('date_pub','>'.$cond);
            }
        }
