...
class UWimage
{
    ...
    public function init()
    {
        return array(
            'name' => __CLASS__,
            'label' => UserModule::t('Image field'),
    ...
    public function viewAttribute($model, $field)
    {
        ...
        if ($file)
        {
            $file = Yii::app()->baseUrl . '/content/profiles/photo/' . $file;

            return CHtml::image($file, $file, array('width'=>100));
        }
        else
            return '';
    }
    ...
    public function editAttribute($model, $field, $params = array())
    {
        ...
        $wrap = '<div class="file-input-box"><input id="fileName" class="file-input-text" readonly="readonly" />';
        return $wrap.CHtml::activeFileField($model, $field->varname, array('size'=>27, 'class'=>"file-input", 'onchange'=>"document.getElementById('fileName').value=this.value;")).'</div>'
